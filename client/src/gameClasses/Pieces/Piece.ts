...
export default abstract class Piece {
  ...
  private startListeningForClicks(): void {
    this.html.addEventListener("mousedown", this.startFollowingCursorMouse);
    this.html.addEventListener("touchstart", this.startFollowingCursorTouch);
  }
  ...
  private startFollowingCursorMouse = (ev: MouseEvent): void => {
    ...
    this.board.showEventsOnBoard.showPossibleMoves(
  ...
  private calcNewTranslateXValue(clientX: number): number {
    return (
      ...
      (this.board.pageContainerHtml.offsetWidth -
        this.board.piecesHtml.offsetWidth) /
        2 -
  ...
  private calcNewTranslateYValue(clientY: number): number {
    return (
      clientY -
      (this.board.pageContainerHtml.offsetHeight -
        this.board.piecesHtml.offsetHeight) /
        2 -
      this.html.offsetWidth / 2
    );
  }
  ...
  private stopFollowingCursorMouse = (
    ...
    const boardGrabbedPieceInfo = this.board
