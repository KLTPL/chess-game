...
export default class FENNotation {
  ...
  private createArrOfPiecesPositionsSeperatedByRows(): string[] {
    ...
    return [
      firstAndLastRowNums.map(pieceNum => this.board.createNewPieceObj(pieceNum, TEAMS.BLACK, this.board)),
      [...Array(FIELDS_IN_ONE_ROW)].map(() => this.board.createNewPieceObj(PIECES.PAWN, TEAMS.BLACK, this.board)),
      ...Array(FIELDS_IN_ONE_ROW/2).fill(
        Array(FIELDS_IN_ONE_ROW).fill(null)
      ),
      [...Array(FIELDS_IN_ONE_ROW)].map(() => this.board.createNewPieceObj(PIECES.PAWN, TEAMS.WHITE, this.board)),
      firstAndLastRowNums.map(pieceNum => this.board.createNewPieceObj(pieceNum, TEAMS.WHITE, this.board))
    ];
  }
