...
export const GET: APIRoute = async ({ request, params }) => {
  try {
    ...
    await queryDB(`INSERT INTO test_time (text) VALUES ('get start')`);
    ...
    const body = new ReadableStream({
      start(controller) {
        queryDB(`INSERT INTO test_time (text) VALUES ('start')`);
      ...
      cancel() {
        queryDB(`INSERT INTO test_time (text) VALUES ('cancel')`);
        OnlineGameController.getInstance(displayId).unsubscribe(sendEventObj);
      },
    });

    await queryDB(`INSERT INTO test_time (text) VALUES ('before return')`);
